...
#             plot_utils.plot_kernel_spike_profile(sp_times[sp_indexes == this_index],
#                                                  signal_kernel_convolutions[this_index],
...
signal_norm_thrs = 1e-4
win_mode = True
sample_numbers = np.array([i for i in range(1, 200)])
...
for s in sample_numbers:
    full_signal = reconstruction_driver.get_signal(sample_number, read_from_wav=False)
        ...
        for k_t in kernel_norm_thresholds:
            for z_mult in z_thres_multiplier:
                ...
                gamma_vals, recons_signal, gamma_vals_manual = iterative_spike_generator. \
                    spike_and_reconstrucviteratively(signal_kernel_convolutions, window_mode=win_mode,
                                                     window_size=w,
                ...
                if not reconstruct_full_signal:
                    recons_coeffs_updated = reconstruction_manager.calculate_reconstruction(sp_times, sp_indexes,
                                                                                            thrs_values)
                else:
                    recons_coeffs_updated = reconstruction_manager. \
                        calculate_reconstruction_in_batch_window_mode(sp_times, sp_indexes,
                                                                      thrs_values, window_size=w,
                                                                      batch_size=spike_batch_size)
                recons = None
                error_rate_fast = None

                error_rate_fast = reconstruction_manager. \
                    calculate_reconstruction_error_rate_fast(recons_coeffs_updated, thrs_values, signal_norm_square)
