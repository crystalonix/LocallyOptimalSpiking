...
def spike_and_reconstruct_iteratively(all_convolutions, window_mode=False, window_size=-1,
    ...
    for t in range(len(all_convolutions[0]) - 1):
        ...
        for i in range(len(all_convolutions)):
            ...
            if t + 1 < len(all_convolutions[i]):
                ...
                ###############################################################
                if not window_mode:
                    # TODO: update the residual norm sq and uncomment
                ...
                #######################################################################
                cur_zeta_val = kernel_projections[i][len(kernel_projections[i]) - 2]

                if cur_zeta_val > norm_threshold_for_new_spike and t > 1 and z_next < z_vals_next[i] \
                    ...
                    if window_mode or z_vals_next[i] > z_thresholds[i]:
                        ...
                        ############ TODO: this is really capturing the previous stage kernel norm ################
                        this_zeta_val = cur_zeta_val
        ...
        if this_spike_index > -1:
            ...
            zeta_values = np.append(zeta_values, np.sqrt(this_zeta_val))
    ...
    return spike_times, spike_indexes, threshold_values, recons_coeffs, z_scores, kernel_projections
...
def update_p_matrix_and_inv(p_matrix, eta_values, zeta_values, beta_values, p_inv_old,
    ...
    eta_values_new = eta_values
    new_norm = zeta_values[-1]
    last_element = 1.0
    if conditioning:
        eta_values_new = (eta_values / zeta_values[:-1]) / new_norm
        last_element = 1.0 / (new_norm * new_norm)
